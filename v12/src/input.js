...
export class Input {
  constructor({ onReset, onCamera, onMenu, onEditor, onDebug, onMusic } = {}) {
    this.keys = new Set();
    this.touch = { left: false, right: false, gas: false, brake: false, hand: false };
    this.pad = { throttle: 0, brake: 0, steer: 0, hand: false };
    this._prevPad = {};
    this.enabled = true;
    const map = { KeyR: onReset, KeyC: onCamera, Escape: onMenu, KeyE: onEditor, F3: onDebug, KeyM: onMusic };
    ...
    this.callbacks = { onReset, onCamera, onMenu };
    this._bindTouch();
  }
  ...
  _bindTouch() {
    ...
    bind('t-left', 'left'); bind('t-right', 'right'); bind('t-gas', 'gas'); bind('t-brake', 'brake'); bind('t-hand', 'hand');
  }

  _poll() {
    const gp = (navigator.getGamepads ? [...navigator.getGamepads()].find((g) => g && g.connected) : null);
    if (!gp) { this.pad.throttle = this.pad.brake = this.pad.steer = 0; this.pad.hand = false; return; }
    ...
    edge(3, this.callbacks.onReset); edge(4, this.callbacks.onCamera); edge(9, this.callbacks.onMenu);
  }
  ...
  read() {
    ...
    return {
      ...
      handbrake: key('Space') || t.hand || this.pad.hand,
    };
  }
}
